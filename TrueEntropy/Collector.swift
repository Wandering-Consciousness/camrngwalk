    ...
    autoreleasepool {
      ...
      if self._lastFrame != nil {  // We recorded lastFrame on previous cycle
        ...
        if s != nil {
          ...
          //soliaxprint("samples count: ",self.samples.count, " byte count: ", self.extractor.pos)
        }
      } else {
        self._lastFrame = f
      }
      let ts = String(format:"%.3f",Date().timeIntervalSince(tStart))
      //soliaxprint("Frame \(self.counter) processed in: \(ts) seconds")
    }
  }
  ...
  func getEntropy() -> [UInt8] {
    return autoreleasepool {
      if self.blockReady() {
        ...
        //soliaxprint("chi square:", self.lastChi)
