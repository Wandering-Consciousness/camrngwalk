...
class Sample {
  ...
  func do_stats() {
    var (sum,dcount) = (Int: 0, Double(self.count))
    var sum_sq: Int = 0 // soliax: split from tuple above
  ...
  func remove_zero_lines(max_zeroes:Int = 7)  {
    ...
    //soliaxprint("removed \(String(format:"%-7d", zRemoved)): \(String(format:"%02.2f",zRemovedPct))% repeat zeroes")
  }
}
